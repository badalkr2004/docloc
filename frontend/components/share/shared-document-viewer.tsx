...
interface SharedDocumentViewerProps {
  ...
  isOpen: boolean;
  onClose: () => void;
  onNavigate: (doc: SharedDocument) => void;
  shareKey: string;
...
export function SharedDocumentViewer({
  ...
  const currentIndex = document ? documents.findIndex(d => d.documentId === document.documentId) : -1;
  const hasPrev = currentIndex > 0;
  const hasNext = currentIndex >= 0 && currentIndex < documents.length - 1;

  // Decrypt and load document using share key
  useEffect(() => {
    if (!isOpen || !document || !shareKey) return;
    ...
    const loadDocument = async () => {
      ...
      try {
        ...
        if (document.presignedUrl.startsWith('http://') || document.presignedUrl.startsWith('https://')) {
          const fileRes = await fetch(document.presignedUrl);
          if (!fileRes.ok) throw new Error('Failed to download encrypted file');
          arrayBuffer = await fileRes.arrayBuffer();
        } else {
          const fileRes = await apiClient.get(document.presignedUrl, { responseType: 'arraybuffer' });
          arrayBuffer = fileRes.data;
        }
        const encryptedBytes = new Uint8Array(arrayBuffer);

        // 2. Unwrap DEK using shareKey (base64url safe)
        const dek = unwrapDekWithShareKey(document.wrappedDekForGrant, shareKey);

        // 3. Decrypt document content
        const decryptedBuffer = await decryptDocumentFile(encryptedBytes, dek);
        ...
        setError(err.message || 'Failed to decrypt document. Share key may be invalid.');
      } finally {
  ...
  }, [document?.documentId, isOpen, shareKey]);
  ...
  const handleDownload = () => {
    if (blobUrl && document && accessType === 'download') {
          ...
          >
            ...
            <div className="flex items-center gap-3 min-w-0 flex-1">
              ...
              <div className="flex items-center gap-2.5 min-w-0">
                <DocTypeIcon docType={(document.docType as any) || 'other'} className="w-5 h-5 shrink-0" />
                  ...
                  <div className="p-5 space-y-6">
                    ...
                    <section>
                      ...
                      <div className="space-y-3">
                        ...
                        {document.docType && (
                          <InfoRow label="Type">
                            <DocTypeBadge docType={document.docType as any} />
                          </InfoRow>
                        )}
                      ...
                      <div className="space-y-3">
                        <div className="flex items-center gap-2 text-sm">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">Decrypted locally via Share Key</p>
                          </div>
                        </div>
                        <InfoRow label="Access Mode" value={accessType === 'download' ? 'View & Download' : 'View Only'} />
                      </div>
                    </section>

...
import { useShareInfo, useVerifyShareOtp } from '@/lib/api/hooks/use-share';
...
export function RecipientView({ token }: { token: string }) {
  ...
  useEffect(() => {
    // Extract key from URL hash (e.g. #key=...)
  ...
  useEffect(() => {
    // If share doesn't require OTP and not yet verified, auto-verify
